...
2. (Optional) change the variables in etc/bootstrap-custom-build.scss
3. Build the Bootstrap custom builds by running `npm install` followed by `npm run build`
4. Overwrite the release files in `src/webapp/dist` with the new files
